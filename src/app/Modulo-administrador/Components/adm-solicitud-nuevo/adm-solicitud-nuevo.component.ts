...
@Component({
  selector: 'app-adm-solicitud-nuevo',
  templateUrl: './adm-solicitud-nuevo.component.html',
  styleUrls: ['./adm-solicitud-nuevo.component.css']
})
export class AdmSolicitudNuevoComponent implements OnInit {
  ...
  editarCompra(solicitud : solicitudDTO){
    ...
    if (solicitud.estado != 'En proceso'){
      ...
      this.form.controls['fechaEstimadaUsuario'].disable();
  ...
  registrarSolicitud() {
    ...
    if(this.modeloGrabar!.idSolicitud == 0){
      ...
      .subscribe({
        next: (res) => {
          if (res.data! > 0) {
            ...
            this.router.navigate(['/main/adm-solicitud']);
          } else {
      ...
      this.subscription = this.solicitudService.actualizaSolicitud(this.modeloGrabar!)
      .subscribe({
        next: (res) => {
          if (res.data! > 0) {
            swal.fire('Éxito', 'Solicitud registrada correctamente', 'success');
